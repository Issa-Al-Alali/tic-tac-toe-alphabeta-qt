...
// Define the players
const char PLAYER_X = 'X'; // AI is X (Maximizing player)
const char PLAYER_O = 'O'; // Human is O (Minimizing player)
const char EMPTY = ' ';
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                ...
                if (board[i][j] == EMPTY) {
                    ...
                    best = max(best, minimax(board, depth + 1, !isMaximizingPlayer, alpha, beta));
            ...
            if (beta <= alpha) {
                // Beta cut-off from inner loop break
                break;
            }
        }
        return best;
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                ...
                if (board[i][j] == EMPTY) {
                    ...
                    best = min(best, minimax(board, depth + 1, !isMaximizingPlayer, alpha, beta));
            ...
            if (beta <= alpha) {
                // Alpha cut-off from inner loop break
                break;
            }
        }
        return best;
...
public:
    TicTacToeWindow(QWidget* parent = nullptr) : QMainWindow(parent) {
        ...
        setWindowTitle("Tic-Tac-Toe with Alpha-Beta Pruning");
